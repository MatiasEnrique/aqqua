...
import { fromYaml } from "@aqqua/shared/schemaYaml";
import { HostProcessPlatform } from "@aqqua/shared/hostProcess";
import { clerkFrontendApiHostnameFromPublishableKey } from "@aqqua/shared/relayAuth";
...
const DESKTOP_APP_ID = "com.aqqua.aqqua";
...
export function resourceMonitorExecutableName(platform: typeof BuildPlatform.Type): string {
  return platform === "win" ? "aqqua-resource-monitor.exe" : "aqqua-resource-monitor";
}
...
interface StagePackageJson {
  ...
  readonly aqquaCommitHash: string;
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // aqqua always passes the user's installed Claude executable to the SDK,
...
export class InvalidAppleTeamIdError extends Schema.TaggedErrorClass<InvalidAppleTeamIdError>()(
  ...
  override get message(): string {
    return `AQQUA_APPLE_TEAM_ID '${this.teamId}' must be a 10-character Apple Developer Team ID.`;
  }
}
...
export class MissingMacPasskeyProvisioningProfileError extends Schema.TaggedErrorClass<MissingMacPasskeyProvisioningProfileError>()(
  ...
  override get message(): string {
    return "AQQUA_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.";
  }
}
...
export class MissingMacPasskeyDomainConfigurationError extends Schema.TaggedErrorClass<MissingMacPasskeyDomainConfigurationError>()(
  ...
  override get message(): string {
    return "AQQUA_CLERK_PUBLISHABLE_KEY or AQQUA_CLERK_PASSKEY_RP_DOMAINS is required for signed macOS passkey builds.";
  }
}
...
export class InvalidMacPasskeyPublishableKeyError extends Schema.TaggedErrorClass<InvalidMacPasskeyPublishableKeyError>()(
  ...
  override get message(): string {
    return "AQQUA_CLERK_PUBLISHABLE_KEY is invalid.";
  }
}
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "AQQUA_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("AQQUA_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "AQQUA_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("AQQUA_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("AQQUA_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("AQQUA_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("AQQUA_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("AQQUA_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  sigma: Config.boolean("AQQUA_DESKTOP_SIGMA").pipe(Config.withDefault(false)),
  verbose: Config.boolean("AQQUA_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
  mockUpdates: Config.boolean("AQQUA_DESKTOP_MOCK_UPDATES").pipe(Config.withDefault(false)),
  mockUpdateServerPort: Config.string("AQQUA_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.option),
  ...
  wslPrebuild: Config.string("AQQUA_DESKTOP_WSL_PREBUILD").pipe(Config.option),
});
 ...
 * `isSigmaDesktopVersion` to pick its bundle id, user data and aqqua home. Keep the
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "aqqua-${version}-${arch}.${ext}",
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      target: [target],
      executableName: "aqqua",
      icon: "icons",
      category: "Development",
      desktop: {
        entry: {
          StartupWMClass: "aqqua",
        },
      },
    };
  }
...
// node-pty's prebuilds/linux-<arch>/ with an aqqua marker the WSL preflight
...
const stageWslNodePtyPrebuild = Effect.fn("stageWslNodePtyPrebuild")(function* (input: {
  ...
  if (input.prebuildPath === undefined) {
    yield* Effect.logWarning(
      "[desktop-artifact] No WSL node-pty prebuild provided (--wsl-prebuild / AQQUA_DESKTOP_WSL_PREBUILD); the packaged WSL backend will not start until a Linux pty.node is bundled.",
    );
    return;
  }
  ...
  yield* fs.writeFileString(path.join(prebuildDir, "aqqua-wsl-node-pty.json"), `${markerJson}\n`);
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `aqqua-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    aqquaCommitHash: commitHash,
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "aqqua desktop build",
    author: "aqqua",
  ...
  yield* runCommand(
    ...
    {
      label: `vp exec --filter @aqqua/desktop -- electron-builder --projectDir ${stageAppDir} ${platformConfig.cliFlag} --${options.arch} --publish never`,
      verbose: options.verbose,
    },
  );
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: AQQUA_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: AQQUA_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: AQQUA_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: AQQUA_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: AQQUA_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `vp run build:desktop` and use existing dist artifacts (env: AQQUA_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: AQQUA_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: AQQUA_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  sigma: Flag.boolean("sigma").pipe(
    Flag.withDescription(
      "Build a parallel Sigma app: own bundle id, own aqqua home, no update feed (env: AQQUA_DESKTOP_SIGMA).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: AQQUA_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: AQQUA_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.integer("mock-update-server-port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Mock update server port (env: AQQUA_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
  wslPrebuild: Flag.string("wsl-prebuild").pipe(
    Flag.withDescription(
      "Path to a prebuilt Linux node-pty (pty.node) for the target arch, staged for the WSL backend (env: AQQUA_DESKTOP_WSL_PREBUILD).",
    ),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for aqqua."),
