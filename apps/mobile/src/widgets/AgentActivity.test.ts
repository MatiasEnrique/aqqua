...
describe("AgentActivity widget layout", () => {
  ...
  it("keeps the brand mark and uses the attention tint for compact status", () => {
    const layout = AgentActivity(
      {
        ...props,
        activeCount: 2,
    ...
    expect(JSON.stringify(layout.compactLeading)).toContain("AqquaMark");
  ...
  it("deep links the banner to the row that needs attention", () => {
    ...
    expect(JSON.stringify(layout.banner)).toContain('"widgetURL":"aqqua://threads/env-1/thread-2"');
  });
  ...
  it("deep links the banner to the first row when nothing needs attention", () => {
    ...
    expect(JSON.stringify(layout.banner)).toContain('"widgetURL":"aqqua://threads/env-1/thread-1"');
  });
