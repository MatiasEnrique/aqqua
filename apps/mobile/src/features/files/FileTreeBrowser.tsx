import type { ProjectEntry } from "@aqqua/contracts";
    ...
    >
      ...
      <Text
        className={cn(
          ...
          props.selected
            ? "font-aqqua-bold text-foreground"
            : "font-aqqua-medium text-foreground-secondary",
        )}
        numberOfLines={1}
      >
        {node.name}
      ...
      {node.kind === "directory" ? (
        <Text className="text-2xs font-aqqua-medium text-foreground-tertiary">
  ...
  if (props.error && props.entries.length === 0) {
    return (
      <View className="flex-1 bg-sheet px-4 py-5">
        <Text className="text-sm font-aqqua-bold text-foreground">Files unavailable</Text>
      ...
      ListEmptyComponent={
        <View className="px-4 py-5">
          {props.isPending ? (
            ...
            <>
              <Text className="text-sm font-aqqua-bold text-foreground">No files found</Text>
