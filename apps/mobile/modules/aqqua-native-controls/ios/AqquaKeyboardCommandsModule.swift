...
public final class AqquaKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("AqquaKeyboardCommands")

    View(AqquaKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: AqquaKeyboardCommandsView, commands: [String]) in
...
public final class AqquaKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.aqquaFirstResponder == nil else { return }
...
private extension UIView {
  var aqquaFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.aqquaFirstResponder { return responder }
    }
    return nil
  }
}
