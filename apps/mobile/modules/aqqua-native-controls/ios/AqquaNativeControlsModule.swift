...
public final class AqquaNativeControlsModule: Module {
  public func definition() -> ModuleDefinition {
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/AqquaShowcaseScene"
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/AqquaShowcaseReadyScene"
      try? scene.write(toFile: readyPath, atomically: true, encoding: .utf8)
    }

    View(AqquaHeaderButtonView.self) {
      Prop("label") { (view: AqquaHeaderButtonView, label: String) in
        view.setLabel(label)
      }
      Prop("systemImage") { (view: AqquaHeaderButtonView, systemImage: String) in
