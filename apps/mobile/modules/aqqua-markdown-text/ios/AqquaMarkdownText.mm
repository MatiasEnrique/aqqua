#import "AqquaMarkdownText.h"
#import "AqquaMarkdownTextShadowNode.h"
#import "AqquaMarkdownTextComponentDescriptor.h"
#import "AqquaMarkdownTextRun.h"
#import <React/RCTConversions.h>
#import <objc/runtime.h>

#import <react/renderer/textlayoutmanager/RCTAttributedTextUtils.h>
#import <react/renderer/components/AqquaMarkdownTextSpec/EventEmitters.h>
#import <react/renderer/components/AqquaMarkdownTextSpec/Props.h>
#import <react/renderer/components/AqquaMarkdownTextSpec/RCTComponentViewHelpers.h>
#import "RCTFabricComponentsPlugins.h"

using namespace facebook::react;

static void AqquaMarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<AqquaMarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void AqquaMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<AqquaMarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = AqquaMarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        AqquaMarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
...
@protocol AqquaMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface AqquaMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<AqquaMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<AqquaMarkdownOutsideTapTarget>)target;

@end

static const void *AqquaMarkdownOutsideTapCoordinatorKey =
    &AqquaMarkdownOutsideTapCoordinatorKey;

@implementation AqquaMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<AqquaMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<AqquaMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, AqquaMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<AqquaMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static AqquaMarkdownOutsideTapCoordinator *
AqquaMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  AqquaMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, AqquaMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[AqquaMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        AqquaMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface AqquaMarkdownText () <RCTAqquaMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface AqquaMarkdownText () <AqquaMarkdownOutsideTapTarget>
@end

@implementation AqquaMarkdownText {
  UIView * _view;
  UITextView * _textView;
  AqquaMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<AqquaMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const AqquaMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    AqquaMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, AqquaMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [AqquaMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<AqquaMarkdownTextProps const>(_props);
  ...
  AqquaMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  AqquaMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::AqquaMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::AqquaMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<AqquaMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<AqquaMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<AqquaMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == AqquaMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == AqquaMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == AqquaMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == AqquaMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const AqquaMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (AqquaMarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[AqquaMarkdownTextRun class]]) {
      continue;
    }

    AqquaMarkdownTextRun* textChild = (AqquaMarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::AqquaMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::AqquaMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> AqquaMarkdownTextCls(void)
{
  return AqquaMarkdownText.class;
}
