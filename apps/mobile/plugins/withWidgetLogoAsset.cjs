...
const IMAGE_SET_CONTENTS =
  JSON.stringify(
    {
      images: [{ idiom: "universal", filename: LOGO_NAME, scale: "1x" }],
...
function withAssetFiles(config) {
  return withDangerousMod(config, [
    "ios",
    (cfg) => {
      const source = path.join(cfg.modRequest.projectRoot, "assets", "widget", LOGO_NAME);
      ...
      fs.copyFileSync(source, path.join(imageSetDir, LOGO_NAME));
      return cfg;
    },
  ]);
}
