...
export interface TelemetryCounters {
  ...
  readonly allAqqua: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allAqqua: ResourceTelemetryAggregate;
  };
...
function applyLifecycleCounters(input: {
  ...
  let allAqqua = input.counters.allAqqua;
  for (const processDelta of input.deltas) {
    ...
    allAqqua = incrementCounters(allAqqua, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allAqqua = incrementCounters(allAqqua, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allAqqua = incrementCounters(allAqqua, { processExits: 1 });
  }

  return { backend, electron, monitor, allAqqua };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allAqqua: aggregate(ordered, counters.allAqqua),
    },
    deltas: processDeltas,
  };
}
