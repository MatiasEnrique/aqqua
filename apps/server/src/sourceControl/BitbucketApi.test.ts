...
const bitbucketPullRequest = {
  ...
  links: {
    html: {
      href: "https://bitbucket.org/pingdotgg/aqqua/pull-requests/42",
    },
  },
  source: {
    branch: { name: "feature/source-control" },
    repository: {
      full_name: "octocat/aqqua",
...
const repositoryJson = {
  full_name: "pingdotgg/aqqua",
  links: {
    html: { href: "https://bitbucket.org/pingdotgg/aqqua" },
    clone: [
      { name: "https", href: "https://bitbucket.org/pingdotgg/aqqua.git" },
      { name: "ssh", href: "git@bitbucket.org:pingdotgg/aqqua.git" },
    ],
  },
...
function makeLayer(input: {
  ...
  const gitMock = {
    readConfigValue: vi.fn<GitVcsDriver.GitVcsDriver["Service"]["readConfigValue"]>(() =>
      Effect.succeed<string | null>("git@bitbucket.org:pingdotgg/aqqua.git"),
    ),
    ...
    listRemotes: () =>
      Effect.succeed({
        remotes: [
          {
            name: "origin",
            url: "git@bitbucket.org:pingdotgg/aqqua.git",
            pushUrl: Option.none(),
            isPrimary: true,
          },
        ],
    ...
    Layer.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            AQQUA_BITBUCKET_API_BASE_URL: "https://api.test.local/2.0",
            AQQUA_BITBUCKET_EMAIL: "user@example.com",
            AQQUA_BITBUCKET_API_TOKEN: "token",
          },
        }),
      ),
    ),
...
it.effect("lists pull requests with Bitbucket state and source branch query params", () => {
        ...
        values: [
          {
            ...
            source: {
              ...
              repository: { full_name: "pingdotgg/aqqua" },
            },
          },
        ],
      }),
  ...
  return Effect.gen(function* () {
    ...
    assert.strictEqual(
      request?.url,
      "https://api.test.local/2.0/repositories/pingdotgg/aqqua/pullrequests",
    );
...
it.effect("reads repository clone URLs and default branch", () => {
  ...
  return Effect.gen(function* () {
    ...
    const cloneUrls = yield* bitbucket.getRepositoryCloneUrls({
      cwd: "/repo",
      repository: "pingdotgg/aqqua",
    });
    const defaultBranch = yield* bitbucket.getDefaultBranch({ cwd: "/repo" });

    assert.deepStrictEqual(cloneUrls, {
      nameWithOwner: "pingdotgg/aqqua",
      url: "https://bitbucket.org/pingdotgg/aqqua.git",
      sshUrl: "git@bitbucket.org:pingdotgg/aqqua.git",
    });
  ...
  () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        [
          "https://api.test.local/2.0/repositories/pingdotgg/aqqua",
          "https://api.test.local/2.0/repositories/pingdotgg/aqqua/branching-model",
        ].toSorted(),
      );
    }).pipe(Effect.provide(layer));
  },
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(JSON.parse(new TextDecoder().decode(rawBody)), {
      ...
      source: {
        ...
        repository: { full_name: "owner/aqqua" },
      },
      destination: {
        branch: { name: "main" },
      },
...
it.effect("checks out fork pull requests through an ensured fork remote", () => {
  const { git, layer } = makeLayer({
    response: (request) => {
      if (request.url.endsWith("/repositories/octocat/aqqua")) {
        return Response.json({
          ...repositoryJson,
          full_name: "octocat/aqqua",
          links: {
            html: { href: "https://bitbucket.org/octocat/aqqua" },
            clone: [
              { name: "https", href: "https://bitbucket.org/octocat/aqqua.git" },
              { name: "ssh", href: "git@bitbucket.org:octocat/aqqua.git" },
            ],
          },
        });
      }
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(git.ensureRemote.mock.calls[0]?.[0], {
      ...
      url: "git@bitbucket.org:octocat/aqqua.git",
    });
    assert.deepStrictEqual(git.fetchRemoteBranch.mock.calls[0]?.[0], {
      cwd: "/repo",
      remoteName: "octocat",
      remoteBranch: "main",
      localBranch: "aqqua/pr-42/main",
    });
    assert.deepStrictEqual(git.setBranchUpstream.mock.calls[0]?.[0], {
      cwd: "/repo",
      branch: "aqqua/pr-42/main",
      remoteName: "octocat",
      remoteBranch: "main",
    });
    assert.deepStrictEqual(git.switchRef.mock.calls[0]?.[0], {
      cwd: "/repo",
      refName: "aqqua/pr-42/main",
    });
  }).pipe(Effect.provide(layer));
});
