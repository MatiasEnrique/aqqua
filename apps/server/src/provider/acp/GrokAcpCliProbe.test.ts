 ...
 * Enable with: AQQUA_GROK_ACP_PROBE=1 bun run test GrokAcpCliProbe
 *
...
const makeProbeRuntime = Effect.gen(function* () {
  ...
  return yield* makeGrokAcpRuntime({
    ...
    childProcessSpawner,
    cwd: process.cwd(),
    clientInfo: { name: "aqqua-grok-probe", version: "0.0.0" },
  });
});

describe.runIf(process.env.AQQUA_GROK_ACP_PROBE === "1")("Grok ACP CLI probe", () => {
