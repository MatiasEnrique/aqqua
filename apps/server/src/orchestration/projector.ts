import type { OrchestrationEvent, OrchestrationReadModel, ThreadId } from "@aqqua/contracts";
import {
...
import { projectBoardEvent } from "./boardProjector.ts";
...
import {
  ...
  ThreadMessageDequeuedPayload,
  ThreadMessageEnqueuedPayload,
...
export function projectEvent(
  ...
  if (event.aggregateKind === "board" || event.aggregateKind === "card") {
  ...
  switch (event.type) {
    ...
    case "thread.turn-diff-completed":
      return Effect.gen(function* () {
        ...
        // worth a warning.
        if (activeTurnOwnsLatest && thread.session.activeTurnId !== payload.turnId) {
        ...
        const turnStillRunning = thread.session?.status === "running";

        return {
          ...nextBase,
          threads: updateThread(nextBase.threads, payload.threadId, {
            checkpoints,
            latestTurn: activeTurnOwnsLatest
              ? thread.latestTurn
              : {
                  turnId: payload.turnId,
