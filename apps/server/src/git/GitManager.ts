...
import { GitManagerError, GitPullRequestMaterializationError } from "@aqqua/contracts";
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `aqqua/pr-${pullRequest.number}/${suffix}`;
}
...
export const make = Effect.gen(function* () {
    ...
    for (const headSelector of headContext.headSelectors) {
      ...
      if (firstPullRequest) {
        return {
          ...
          state: "open",
          ...(firstPullRequest.hasConflicts !== undefined
            ? { hasConflicts: firstPullRequest.hasConflicts }
            : {}),
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(
      tempDir,
      `aqqua-pr-body-${process.pid}-${yield* randomUUIDv4(cwd)}.md`,
    );
