...
export const BoardCompleteTool = Tool.make("board_complete", {
  description:
    "Signal that this board step thread has finished. Call with outcome `success` when the step's work is done (and its artifact is written), or `blocked` when you cannot proceed and need human input. Only the card's current step thread may call this; sub-agents and ordinary chats are refused.",
