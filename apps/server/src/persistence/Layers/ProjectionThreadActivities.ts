...
import { NonNegativeInt } from "@aqqua/contracts";
...
const makeProjectionThreadActivityRepository = Effect.gen(function* () {
  ...
  const listProjectionThreadUserInputRows = SqlSchema.findAll({
    Request: ListProjectionThreadActivitiesInput,
    Result: ProjectionThreadActivityDbRowSchema,
    execute: ({ threadId }) =>
      sql`
  ...
  const listUserInputByThreadId: ProjectionThreadActivityRepositoryShape["listUserInputByThreadId"] =
    (input) =>
      listProjectionThreadUserInputRows(input).pipe(
        Effect.mapError(
          toPersistenceSqlOrDecodeError(
            "ProjectionThreadActivityRepository.listUserInputByThreadId:query",
            "ProjectionThreadActivityRepository.listUserInputByThreadId:decodeRows",
          ),
        ),
        Effect.map((rows) =>
          rows.map((row) => ({
