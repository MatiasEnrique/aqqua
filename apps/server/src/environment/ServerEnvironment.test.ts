...
it.layer(NodeServices.layer)("ServerEnvironmentLive", (it) => {
  it.effect("persists the environment id across service restarts", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "aqqua-server-environment-test-",
      });
      ...
      expect(second.capabilities.boardCardReset).toBe(true);
      expect(second.capabilities.boardCardForceAdvance).toBe(true);
      expect(second.capabilities.threadMessageQueue).toBe(true);
      expect(second.capabilities.threadMessageQueueSteering).toBe(true);
    }),
  );
  ...
  it.effect("structures persisted environment id filesystem failures", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "aqqua-server-environment-error-test-",
      });
