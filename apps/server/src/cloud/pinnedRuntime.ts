...
/**
 * A pinned runtime is an exact `aqqua@<version>` npm-installed into
 ...
 * switching over — never `npx aqqua`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "aqqua", "dist", "bin.mjs"),
...
/**
 * Installs `aqqua@<version>` into the pinned runtime directory unless a complete
  ...
  function* (input: {
    ...
    return yield* pinnedRuntimeInstallLock.withPermit(
      Effect.gen(function* () {
        ...
        const installStep = "installing the pinned aqqua runtime (this can take a few minutes)";
        yield* runner
          .run({
            command: "npm",
            args: [
