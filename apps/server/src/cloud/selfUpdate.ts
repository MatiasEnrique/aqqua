...
/**
 * Lets a connected client replace this server with another published `aqqua`
...
export function isPublishedCliEntry(entryPath: string): boolean {
  return normalizeEntryPath(entryPath).includes("/node_modules/aqqua/dist/");
}

/**
 * The update path this process can offer, or null when only a manual
 * relaunch works. "desktop-managed" — the aqqua desktop app spawned this
...
export const resolveServerSelfUpdateCapability = Effect.fn(
  ...
  if (platform === "linux" && homeDir !== "") {
    ...
    // explicit marker written into aqqua.service identifies this unit as the
...
>()("aqqua/cloud/selfUpdate/ServerSelfUpdate") {}
  ...
  const update: ServerSelfUpdate["Service"]["update"] = Effect.fn(
    ...
    if (capability === "desktop-managed") {
      return yield* failWith(
        "This server is managed by the aqqua desktop app on its machine; update the desktop app to update it.",
      );
    }
    if (capability === null) {
      return yield* failWith(
    ...
    if (!EXACT_VERSION_PATTERN.test(targetVersion)) {
      return yield* failWith(`'${targetVersion}' is not an exact aqqua version.`);
    }
    ...
    return yield* Effect.gen(function* () {
      const runtimePaths = yield* ensurePinnedRuntimeInstalled({
        ...
        Effect.mapError((error) =>
          failWith("Could not install the requested aqqua version.", error),
        ),
      );
      ...
      const preflight = yield* runner
        ...
        .pipe(
          Effect.mapError((cause) =>
            failWith(`Could not verify the installed aqqua@${targetVersion}.`, cause),
          ),
        );
      ...
      if (preflight.code !== 0 || reportedVersion !== targetVersion) {
        ...
        yield* removePinnedRuntimeInstallation({
          ...
          Effect.mapError((error) =>
            failWith(`Could not remove the failed aqqua@${targetVersion} installation.`, error),
          ),
        );
        return yield* failWith(
          preflight.code !== 0
            ? `The installed aqqua@${targetVersion} failed its version check (exit code ${String(preflight.code)}).`
            : `The installed runtime did not report the requested aqqua@${targetVersion} version.`,
        );
      }
      ...
      if (activeMethod === "boot-service") {
        ...
        const unit = renderBootServiceUnit({
          ...
          aqquaEntryPath: runtimePaths.entryPath,
          baseDir: serverConfig.baseDir,
        ...
        yield* host
          ...
          .pipe(
            Effect.mapError((cause) =>
              failWith("Could not start the replacement aqqua process.", cause),
            ),
          );
        ...
        yield* scheduleRestart(
          Effect.try({
            ...
            catch: (cause) => failWith("Could not exit the replaced aqqua process.", cause),
          }).pipe(
            Effect.catch((error) =>
