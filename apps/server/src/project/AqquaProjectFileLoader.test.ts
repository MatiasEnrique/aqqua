...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(AqquaProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "aqqua.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("AqquaProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid aqqua.json", () =>
      Effect.gen(function* () {
        const loader = yield* AqquaProjectFileLoader.AqquaProjectFileLoader;
    ...
    it.effect("returns none when aqqua.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* AqquaProjectFileLoader.AqquaProjectFileLoader;
        const cwd = yield* makeTempDir;
