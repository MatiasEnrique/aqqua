...
const shouldLaunchElectronAfterPack = process.env.AQQUA_DESKTOP_DEV === "1";
  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["aqqua#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-preview-annotation-css.mjs && cross-env AQQUA_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["aqqua#build"],
        cache: false,
      },
      "dev:bundle": {
  ...
  pack: [
    {
      ...
      clean: true,
      deps: {
        alwaysBundle: (id) => id.startsWith("@aqqua/"),
      },
