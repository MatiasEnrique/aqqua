...
import { EnvironmentId, type PersistedSavedEnvironmentRecord } from "@aqqua/contracts";
...
function makeLayer(
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ AQQUA_HOME: baseDir })),
    ),
  );
...
const withSavedEnvironments = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "aqqua-desktop-saved-environments-test-",
    });
