...
const OVERLAY_ATTRIBUTE = "data-aqqua-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--aqqua-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--aqqua-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--aqqua-radius": theme.radius,
    "--aqqua-background": theme.background,
    "--aqqua-foreground": theme.foreground,
    "--aqqua-popover": theme.popover,
    "--aqqua-popover-foreground": theme.popoverForeground,
    "--aqqua-primary": theme.primary,
    "--aqqua-primary-foreground": theme.primaryForeground,
    "--aqqua-muted": theme.muted,
    "--aqqua-muted-foreground": theme.mutedForeground,
    "--aqqua-accent": theme.accent,
    "--aqqua-accent-foreground": theme.accentForeground,
    "--aqqua-border": theme.border,
    "--aqqua-input": theme.input,
    "--aqqua-ring": theme.ring,
    "--aqqua-font-sans": theme.fontSans,
    "--aqqua-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-aqqua-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--aqqua-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-aqqua-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
