...
import type { DesktopUpdateState } from "@aqqua/contracts";
...
function makeHarness(options: UpdatesHarnessOptions = {}) {
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    homeDirectory: `/tmp/aqqua-desktop-updates-home-${process.pid}`,
    platform: "darwin",
    processArch: "x64",
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          AQQUA_HOME: `/tmp/aqqua-desktop-updates-test-${process.pid}`,
          AQQUA_DESKTOP_MOCK_UPDATES: "true",
          AQQUA_DESKTOP_MOCK_UPDATE_SERVER_PORT: "4141",
          ...options.env,
        }),
      ),
    ),
...
describe("DesktopUpdates", () => {
  ...
  it.effect("normalizes release notes and broadcasts summaries", () => {
    const harness = makeHarness();
  ...
  it.effect("recovers download state after an unexpected setup failure", () => {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        const unchangedState = yield* updates.setChannel("latest");
        assert.equal(unchangedState.channel, "latest");
      }),
    ).pipe(Effect.provide(Layer.merge(TestClock.layer(), harness.layer)));
  });
