...
import * as SshAuth from "@aqqua/ssh/auth";
import { discoverSshHosts } from "@aqqua/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@aqqua/ssh/errors";
import * as SshTunnel from "@aqqua/ssh/tunnel";
...
>()("@aqqua/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliPackageSpec?: () => string;
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemoteAqquaRunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "aqqua window is not available for SSH authentication.";
      break;
