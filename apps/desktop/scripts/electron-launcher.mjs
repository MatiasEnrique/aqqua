// This file mostly exists because we want dev mode to say "Aqqua (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Aqqua (Dev)" : "Aqqua (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.aqqua.aqqua.dev.${devBundleIdSuffix || "local"}`
  : "com.aqqua.aqqua";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["aqqua-dev"] : ["aqqua"];
...
// oxlint-disable-next-line aqqua/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["AQQUA_PORT", environment.AQQUA_PORT],
    ["AQQUA_HOME", environment.AQQUA_HOME],
    ["AQQUA_COMMIT_HASH", environment.AQQUA_COMMIT_HASH],
    ["AQQUA_OTLP_TRACES_URL", environment.AQQUA_OTLP_TRACES_URL],
    ["AQQUA_OTLP_EXPORT_INTERVAL_MS", environment.AQQUA_OTLP_EXPORT_INTERVAL_MS],
    ["AQQUA_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --aqqua-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the aqqua name.
