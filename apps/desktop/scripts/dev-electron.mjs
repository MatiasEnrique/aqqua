...
const remoteDebuggingPort = process.env.AQQUA_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line aqqua/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.AQQUA_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.AQQUA_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--aqqua-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--aqqua-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
