...
import { DEV_PROXIED_PATH_PREFIXES } from "@aqqua/shared/devProxy";
...
const isSingleOriginDev = process.env.AQQUA_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_AQQUA_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.AQQUA_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: AQQUA_BUNDLED_DEV=1 pnpm dev:web
const bundledDevEnv = process.env.AQQUA_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // AQQUA_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.AQQUA_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.AQQUA_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_AQQUA_RELAY_URL": JSON.stringify(configuredRelayUrl),
