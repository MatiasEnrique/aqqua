import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@aqqua/contracts";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "aqqua:version-mismatch-dismissals:v1";
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same aqqua version.",
  };
}
...
export function manualServerUpdateCommand(targetVersion: string): string {
  return `npx aqqua@${targetVersion}`;
}
...
export function serverUpdateGuidance(
  ...
  switch (capability) {
    ...
    case "desktop-managed":
      return `The ${serverLabel} is run by the aqqua desktop app on its machine — update the desktop app there to sync them.`;
    default:
