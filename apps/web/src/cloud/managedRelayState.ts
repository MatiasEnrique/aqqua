...
} from "@aqqua/client-runtime/relay";
import type { RelayClientDeviceRecord, RelayClientEnvironmentRecord } from "@aqqua/contracts/relay";
...
export function useManagedRelayEnvironments() {
  ...
  useEffect(() => {
    if (snapshot.error) {
      console.error("[aqqua-cloud] Relay environment listing failed", {
...
export function useManagedRelayDevices() {
  ...
  useEffect(() => {
    if (snapshot.error) {
      console.error("[aqqua-cloud] Relay device listing failed", {
        message: snapshot.error,
        traceId: snapshot.errorTraceId,
      });
    }
