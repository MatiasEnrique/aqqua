import { DEFAULT_CLIENT_SETTINGS } from "@aqqua/contracts";
...
describe("clientPersistenceStorage", () => {
  it("persists client settings in browser storage", async () => {
    ...
    const settings = {
      ...
      headerTabScope: "worktree" as const,
  ...
  it("reports structured decode failures while preserving the fallback", async () => {
    ...
    testWindow.localStorage.setItem("aqqua:client-settings:v1", "not-json");
    ...
    expect(consoleError).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        operation: "decode",
        storageKey: "aqqua:client-settings:v1",
