...
describe("resolveClerkSignInProps", () => {
  it("returns to the current browser URL on the web", () => {
    const href = "https://app.aqqua.codes/connect?state=state-1#details";
  ...
  it("omits the redirect override on packaged desktop", () => {
    expect(resolveClerkSignInProps("aqqua://app/#/settings/general", true)).toEqual({});
  });

  it("omits the redirect override on development desktop", () => {
    expect(resolveClerkSignInProps("aqqua-dev://app/#/settings/general", true)).toEqual({});
  });
});
