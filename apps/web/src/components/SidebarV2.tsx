...
export default function SidebarV2() {
  ...
  const handleMultiSelectContextMenu = useCallback(
    async (position: { x: number; y: number }) => {
      ...
      const targets = threadKeys.flatMap((threadKey) => {
        const thread = threadByKeyRef.current.get(threadKey);
        return thread ? [thread] : [];
      });
      const result = await deleteThreads(targets);
      if (result._tag === "Failure") {
        if (!isAtomCommandInterrupted(result)) {
          const error = squashAtomCommandFailure(result);
          toastManager.add(
      ...
      if (result.value !== null) removeFromSelection([...result.value]);
    },
    [
      attemptSettle,
      attemptSnooze,
  ...
  // button): confirmation and worktree cleanup stay in useThreadActions.
