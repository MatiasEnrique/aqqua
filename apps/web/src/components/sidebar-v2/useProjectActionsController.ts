...
import type { ProjectActionsController, SidebarProjectsSection } from "./models";
import type { SidebarV2Runtime } from "./useSidebarV2Sections";

export function useProjectActionsController(input: {
  readonly projects: SidebarProjectsSection;
  readonly runtime: SidebarV2Runtime;
}): ProjectActionsController {
  const { projects, runtime } = input;
  const {
    deleteProject,
    updateProject,
    updateSettings,
