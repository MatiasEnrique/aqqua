...
import type { EnvironmentConnectionPresentation } from "@aqqua/client-runtime/connection";
import { serializeComposerFileLink } from "@aqqua/shared/composerTrigger";
import { createModelSelection, normalizeModelSlug } from "@aqqua/shared/model";
import {
  memo,
  type ReactNode,
  useCallback,
...
import type { UnifiedSettings } from "@aqqua/contracts/settings";
  ...
  return (
    <>
      ...
      <ComposerPrimaryActions
        ...
        messageQueueSupported={props.messageQueueSupported}
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const workspaceSkills = useProviderWorkspaceSkills(
    {
      environmentId,
      instanceId: noProviderAvailable ? null : selectedInstanceId,
      cwd: gitCwd,
    },
  ...
  const composerMenuItems = useMemo<ComposerCommandItem[]>(() => {
    ...
    if (composerTrigger.kind === "skill") {
      return searchProviderSkills(workspaceSkills.skills, composerTrigger.query).map((skill) => ({
        id: providerSkillStableId(skill, selectedProvider),
        type: "skill" as const,
        provider: selectedProvider,
        skill,
  ...
  const composerFooterHasWideActions =
    phase === "running" || showPlanFollowUpPrompt || activePendingProgress !== null;
  ...
  const isComposerMenuLoading =
    ...
    (composerTriggerKind === "skill" && workspaceSkills.isPending);
  const composerMenuEmptyState = useMemo(() => {
    if (composerTriggerKind === "skill") {
      if (workspaceSkills.error) {
        return workspaceSkills.error;
      }
    ...
    return composerTriggerKind === "path"
      ...
      : "No matching command.";
  }, [composerTriggerKind, workspaceSkills.error]);
  ...
  return (
    ...
    >
      <div
        className={cn(
          "group rounded-[var(--chat-composer-radius,10px)] p-px transition-colors duration-200",
      ...
      >
        <div
          ...
          className={cn(
            "rounded-[var(--chat-composer-inner-radius,9px)] transition-[background-color] duration-200",
        ...
        >
          {!isComposerCollapsedMobile && (activeThread?.queuedMessages?.length ?? 0) > 0 ? (
          ...
          {!isComposerCollapsedMobile &&
            (activePendingApproval ? (
              <div className="rounded-t-[var(--chat-composer-inner-radius,9px)] border-b border-border/65 bg-muted/20">
          ...
          {isComposerCollapsedMobile && activePendingApproval ? (
            <div
              className="rounded-t-[var(--chat-composer-inner-radius,9px)] border-b border-border/65 bg-muted/20"
          ...
          <div
            ...
            data-chat-composer-prompt-block="true"
            className={cn(
              "relative px-3 pb-3 sm:px-4",
                      ...
                      >
                        {image.type === "image" && image.previewUrl ? (
                          ...
                          <div className="flex h-full w-full flex-col items-center justify-center gap-1 px-1 text-center text-[10px] text-muted-foreground/70">
                            {image.type === "file" ? <FileIcon className="size-5" /> : null}
              ...
              <div>
                ...
                {showMobilePendingAnswerActions ? (
                  <div
                    ...
                    className="absolute right-0 bottom-0 flex justify-end"
                  >
                    <ComposerPrimaryActions
                      compact
                      pendingAction={pendingPrimaryAction}
