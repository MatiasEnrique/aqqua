...
import { CHAT_LIST_ANCHOR_OFFSET } from "@aqqua/shared/chatList";
import { projectScriptCwd, projectScriptRuntimeEnv } from "@aqqua/shared/projectScripts";
import { truncate } from "@aqqua/shared/String";
import { nextTerminalId, resolveTerminalSessionLabel } from "@aqqua/shared/terminalLabels";
...
import { resolveProviderWorkspaceSkillsInstanceId } from "@aqqua/client-runtime/state/provider-skills";
...
function ChatViewContent(props: ChatViewProps) {
  ...
  const discardConversationDraftTab = useCallback(
    (nextDraftId: string) => {
      const closingTab = conversationTabsRef.current.find(
        (tab) => tab._tag === "draft" && tab.draftId === nextDraftId,
      );
      const target = closingTab
        ? resolveConversationTabAfterClose({
            tabs: conversationTabsRef.current,
            closingKey: closingTab.key,
          })
        : null;
    ...
    [navigate, navigateAfterConversationClose],
  );
  ...
  const compactViewport = useMediaQuery(RIGHT_PANEL_INLINE_LAYOUT_MEDIA_QUERY);
  const [workspaceElement, setWorkspaceElement] = useState<HTMLDivElement | null>(null);
  const [workspaceCompact, setWorkspaceCompact] = useState<boolean | undefined>(undefined);
  useLayoutEffect(() => {
    if (!workspaceElement) return;
    const updateCompactState = () => {
      const next = workspaceElement.clientWidth < 800;
      setWorkspaceCompact((current) => (current === next ? current : next));
    };
    updateCompactState();
  ...
  const shouldUsePlanSidebarSheet = compactViewport || workspaceCompact === true;
  ...
  const activeWorkspacePanelRef = useMemo(
    () =>
      activeThreadRef && (activeThreadWorktreePath ?? activeProjectCwd)
        ? {
            environmentId: activeThreadRef.environmentId,
            workspaceRoot: activeThreadWorktreePath ?? activeProjectCwd!,
          }
        : null,
    [activeProjectCwd, activeThreadRef, activeThreadWorktreePath],
  );
  const rightPanelContext = useMemo(
    () =>
      activeThreadRef
        ? { threadRef: activeThreadRef, workspaceRef: activeWorkspacePanelRef }
        : null,
  ...
  const preferredRightPanelSurfaceIdsRef = useRef(new Map<string, string>());
  const preferredRightPanelSurfaceIdForKind = useCallback(
    (kind: RightPanelSurface["kind"]) => {
      if (!rightPanelContext) return undefined;
      return preferredRightPanelSurfaceIdsRef.current.get(
        rightPanelSelectionMemoryKey(rightPanelContext, kind),
      );
    },
    [rightPanelContext],
  );
  useEffect(() => {
    if (!rightPanelContext || rightPanelState.activeSurfaceId === null) return;
    const activeSurface = rightPanelState.surfaces.find(
      (surface) => surface.id === rightPanelState.activeSurfaceId,
    );
    if (!activeSurface) return;
    preferredRightPanelSurfaceIdsRef.current.set(
      rightPanelSelectionMemoryKey(rightPanelContext, activeSurface.kind),
      activeSurface.id,
    );
  }, [rightPanelContext, rightPanelState.activeSurfaceId, rightPanelState.surfaces]);
  ...
  const supportsWorkspaceTerminalSessions =
    activeEnvironment?.serverConfig?.environment.capabilities.workspaceTerminalSessions === true;
  const panelTerminalThreadRef =
    !supportsWorkspaceTerminalSessions &&
    activeThreadRef &&
    activeRightPanelSurface?.kind === "terminal" &&
    activeRightPanelSurface.originThreadId
      ? scopeThreadRef(
  ...
  const closePreviewPanel = useCallback(() => {
    if (!activeThreadRef || !rightPanelContext) return;
    setMaximizedRightPanelThreadKey(null);
    useRightPanelStore.getState().hideContext(rightPanelContext);
  }, [activeThreadRef, rightPanelContext]);
  const addTerminalSurface = useCallback(() => {
    ...
    void openTerminal({
      ...
      input: {
        ...
        terminalId,
        cwd,
        ...(supportsWorkspaceTerminalSessions && activeWorkspacePanelRef
          ? { workspaceRoot: activeWorkspacePanelRef.workspaceRoot }
          : {}),
        ...(activeThreadWorktreePath != null ? { worktreePath: activeThreadWorktreePath } : {}),
        env: projectScriptRuntimeEnv({
          project: { cwd: activeProject.workspaceRoot },
  ...
  const closePanelTerminal = useCallback(
    (terminalId: string) => {
      ...
      void closeTerminalMutation({
        environmentId: activeThreadRef.environmentId,
        input: {
          threadId: panelTerminalThreadRef.threadId,
          terminalId,
          deleteHistory: true,
          ...(supportsWorkspaceTerminalSessions && activeWorkspacePanelRef
            ? { workspaceRoot: activeWorkspacePanelRef.workspaceRoot }
  ...
  const toggleRightPanel = useCallback(() => {
    ...
    if (rightPanelContext) {
      useRightPanelStore.getState().restoreContext(rightPanelContext);
    }
  }, [
    activeThreadRef,
    closePreviewPanel,
  ...
  const openRightPanelSurface = useCallback(
    (kind: RightPanelSurfaceButtonKind) => {
      if (kind === "browser" || kind === "terminal") {
        ...
        const existing = resolveActivitySurface({
          surfaces: rightPanelState.surfaces,
          kind: wantedKind,
          preferredId: preferredRightPanelSurfaceIdForKind(wantedKind),
        });
        if (existing) {
    ...
    [
      ...
      addTerminalSurface,
      createBrowserSurface,
      preferredRightPanelSurfaceIdForKind,
      rightPanelState.surfaces,
    ],
  );
  ...
  const cleanupRightPanelSurfaces = useCallback(
    (surfaces: readonly RightPanelSurface[]) => {
      ...
      for (const surface of surfaces) {
        ...
        if (surface.kind === "terminal") {
          for (const terminalId of surface.terminalIds) {
            const pane = surface.terminalPanes.find(
              (candidate) => candidate.terminalId === terminalId,
            );
            const terminalThreadRef =
              !supportsWorkspaceTerminalSessions && pane?.originThreadId
                ? scopeThreadRef(activeThreadRef.environmentId, ThreadId.make(pane.originThreadId))
                : (panelTerminalThreadRef ?? activeThreadRef);
            ...
            void closeTerminalMutation({
              ...
              input: {
                ...
                deleteHistory: true,
                ...(supportsWorkspaceTerminalSessions &&
                (pane?.workspaceRoot ?? activeWorkspacePanelRef?.workspaceRoot)
                  ? {
                      workspaceRoot:
                        pane?.workspaceRoot ?? activeWorkspacePanelRef?.workspaceRoot ?? "",
                    }
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    const turnAttachmentsPromise = Promise.all(
      composerImagesSnapshot.map(async (attachment) => {
        ...
        return attachment.type === "image"
          ? { type: "image" as const, ...upload }
          : { type: "file" as const, ...upload };
      }),
    );
    const optimisticAttachments = composerImagesSnapshot.map((attachment) => {
      ...
      return attachment.type === "image"
        ? { type: "image" as const, ...optimistic }
        : { type: "file" as const, ...optimistic };
    });
  ...
  const rightPanelContent = activeThreadRef ? (
    activeRightPanelSurface?.kind === "preview" ? (
      ...
      <PersistentThreadTerminalPanel
        threadRef={panelTerminalThreadRef ?? activeThreadRef}
        {...(supportsWorkspaceTerminalSessions && activeWorkspacePanelRef
          ? { workspaceRoot: activeWorkspacePanelRef.workspaceRoot }
          : {})}
  ...
  const workspaceActions = (
    <ChatHeader
      ...
      keybindings={keybindings}
      availableEditors={availableEditors}
      rail={!shouldUsePlanSidebarSheet}
      gitCwd={gitCwd}
  ...
  return (
    <div
      ...
      className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-sidebar"
    >
      ...
      >
        ...
        />
        {/* Main content area with optional plan sidebar. Stays a fully rounded
            slab even beside the right panel: the two surfaces read as separate
            cards on the chrome, with the resize gutter between them. */}
        <div className="flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background md:rounded-xl">
          {/* Chat column */}
      ...
      {!shouldUsePlanSidebarSheet && activeThreadRef ? (
        <RightPanelSidebar
          mode="inline"
          projectActions={workspaceActions}
          ...
          preferredSurfaceIdForKind={preferredRightPanelSurfaceIdForKind}
          onActivate={activateRightPanelSurface}
          onCloseSurface={closeRightPanelSurface}
          onCloseOtherSurfaces={closeOtherRightPanelSurfaces}
          onCloseSurfacesToRight={closeRightPanelSurfacesToRight}
          onCloseAllSurfaces={closeAllRightPanelSurfaces}
          onHide={closePreviewPanel}
        ...
        >
          {rightPanelOpen ? rightPanelContent : null}
        </RightPanelSidebar>
      ) : null}
