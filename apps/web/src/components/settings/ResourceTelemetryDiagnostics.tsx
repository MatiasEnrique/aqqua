...
export function ResourceTelemetryDiagnostics() {
  ...
  const allAqqua = snapshot?.groups.allAqqua;
        ...
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03),0_8px_30px_rgb(0_0_0/0.035)]">
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-border/55 md:grid-cols-3">
            <IconStat
              ...
              label="Current CPU"
              value={allAqqua ? `${allAqqua.currentCpuPercent.toFixed(1)}%` : "..."}
              detail={
                allAqqua ? `${formatCpuTime(allAqqua.cpuTimeMs)} observed CPU time` : undefined
              }
            />
            <IconStat
              icon={<MemoryStickIcon className="size-3.5" />}
              label="Resident memory"
              value={allAqqua ? formatBytes(allAqqua.currentRssBytes) : "..."}
              detail={
                allAqqua
                  ? `${formatBytes(allAqqua.peakRssBytes)} combined process peaks`
                  : undefined
              }
            />
            <IconStat
              ...
              label="Process count"
              value={allAqqua ? String(allAqqua.processCount) : "..."}
              detail={
                allAqqua
                  ? `${allAqqua.processStarts} starts · ${allAqqua.processExits} exits`
                  : undefined
              }
            />
            <IconStat
              ...
              value={allAqqua ? formatRate(allAqqua.ioReadBytesPerSecond) : "..."}
              detail={allAqqua ? `${formatBytes(allAqqua.ioReadBytes)} observed` : undefined}
            />
            <IconStat
              icon={<DatabaseIcon className="size-3.5" />}
              label="Write throughput"
              value={allAqqua ? formatRate(allAqqua.ioWriteBytesPerSecond) : "..."}
              detail={allAqqua ? `${formatBytes(allAqqua.ioWriteBytes)} observed` : undefined}
              tone={
                allAqqua && allAqqua.ioWriteBytesPerSecond >= 10 * 1_024 * 1_024
                  ? "danger"
                  : allAqqua && allAqqua.ioWriteBytesPerSecond >= 1_024 * 1_024
                    ? "warning"
                    : "default"
              }
            />
      ...
      >
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03)]">
          <div className="bg-muted/15 px-4 py-3 text-[11px] leading-relaxed text-muted-foreground sm:px-5">
            ...
            counters identify known aqqua operations so process spikes can be correlated with
