...
import type { EnvironmentId, ServerSelfUpdateCapability } from "@aqqua/contracts";
import {
 ...
 * (running `npx aqqua` there would start a second server, not update this one),
    ...
    void Promise.resolve()
      ...
      .then((result) => {
        ...
        toastManager.add({
          ...
          description: `aqqua@${result.value.targetVersion} is installed — the server is restarting and will reconnect shortly.`,
        });
      })
      .catch((error: unknown) => {
        if (!ownsAttempt()) return;
