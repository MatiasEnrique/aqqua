...
describe("hostedPairing", () => {
  ...
  it("reads hosted pairing host and query token parameters", () => {
    const url = new URL("https://app.aqqua.codes/pair?host=100.64.1.2:3773&token=ABCD1234");
  ...
  it("prefers hash tokens so generated hosted links do not put credentials in search params", () => {
    ...
    expect(url.origin).toBe("https://preview.aqqua.codes");
  ...
  it("ignores incomplete hosted pairing requests", () => {
    ...
    expect(hasHostedPairingRequest(new URL("https://app.aqqua.codes/pair?token=ABCD1234"))).toBe(
      false,
    );
  });
  ...
  it("detects the hosted static app only when no backend URL is configured", () => {
    ...
    expect(isHostedStaticApp(new URL("https://preview.aqqua.codes/"))).toBe(true);
    expect(isHostedStaticApp(new URL("https://preview.aqqua.codes/pair"))).toBe(true);
