...
- `aqqua-relay-traces-prod`, an OpenTelemetry trace dataset for Worker requests
- `aqqua-relay-otel-ingest-prod`, a dataset-scoped ingest token bound to the Worker
- `aqqua-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`aqqua-relay-traces-dev-julius` for a personal stage.
