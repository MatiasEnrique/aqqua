...
aqqua has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.aqqua/userdata/logs/server.trace.ndjson`).
...
```bash
export AQQUA_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export AQQUA_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export AQQUA_OTLP_SERVICE_NAME=aqqua-local
```

Optional:

```bash
export AQQUA_TRACE_MIN_LEVEL=Info
export AQQUA_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `AQQUA_OTLP_*`.

macOS app bundle example:

```bash
AQQUA_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
AQQUA_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
AQQUA_OTLP_SERVICE_NAME=aqqua-desktop \
...
```bash
TRACE_FILE="${AQQUA_HOME:-$HOME/.aqqua}/userdata/logs/server.trace.ndjson"
```
...
```bash
TRACE_FILE="$HOME/.aqqua/dev/logs/server.trace.ndjson"
tail -f "$TRACE_FILE"
```
...
- service name such as `aqqua-local`, `aqqua-dev`, or `aqqua-desktop`
...
- `aqqua_rpc_request_duration`
- `aqqua_orchestration_command_duration`
- `aqqua_orchestration_command_ack_duration`
- `aqqua_provider_turn_duration`
- `aqqua_git_command_duration`
- `aqqua_db_query_duration`

Counters tell you volume and failure rate:

- `aqqua_rpc_requests_total`
- `aqqua_orchestration_commands_total`
- `aqqua_provider_turns_total`
- `aqqua_git_commands_total`
- `aqqua_db_queries_total`
...
`aqqua_orchestration_command_ack_duration` measures:
...
1. Check `aqqua_orchestration_command_ack_duration` by `commandType`.
...
- `AQQUA_OTLP_TRACES_URL` was not set
...
- `AQQUA_TRACE_FILE`: override trace file path
- `AQQUA_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `AQQUA_TRACE_MAX_FILES`: rotated file count, default `10`
- `AQQUA_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `AQQUA_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `AQQUA_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `AQQUA_OTLP_TRACES_URL`: OTLP trace endpoint
- `AQQUA_OTLP_METRICS_URL`: OTLP metric endpoint
- `AQQUA_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `AQQUA_OTLP_SERVICE_NAME`: service name, default `aqqua-server`
