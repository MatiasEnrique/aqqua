...
The screenshot harness runs the real mobile application against three disposable local aqqua
...
the software keyboard, and supplies deterministic aqqua Connect discovery rows to the real
...
1. Creates three temporary aqqua base directories and starts a local server for each on an available
   port.
2. Creates aqqua, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic aqqua review diff.
...
ARM64 by default for local Apple Silicon development; CI sets `AQQUA_SHOWCASE_ANDROID_ABI=x86_64` so the
...
Run Metro separately:
    ...
    pnpm --filter @aqqua/mobile showcase
...
The Pending rows use the production offline outbox and point at the real aqqua and React fixture
