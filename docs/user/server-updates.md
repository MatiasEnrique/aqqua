# Keeping aqqua in Sync

The aqqua web or desktop app and the server it connects to work best when they use the same
version. If they do not match, aqqua shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Select the button and leave aqqua open. It prepares the matching version, restarts the server, and reconnects automatically. This can take several minutes.               |
| **Update the desktop app** | Open the aqqua desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current aqqua server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. aqqua does not update connected
servers silently in the background.

If the server uses the aqqua background service, you can also update it directly on the host:

```sh
npx aqqua@latest service update
```

See [Running aqqua in the Background](./background-service.md) for install, status, and removal
commands.
...
3. For a command-line server, relaunch it with `npx aqqua@<client-version>`, replacing
