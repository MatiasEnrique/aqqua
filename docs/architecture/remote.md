...
This document describes the target architecture for first-class remote environments in aqqua.
...
- Keep the aqqua server as the execution boundary.
...
aqqua already has a clean runtime boundary: the client talks to an aqqua server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one aqqua server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one aqqua server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the aqqua runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running aqqua server instance.
...
How does the client speak WebSocket to an aqqua server?
...
- best fit for hosted or self-managed remote aqqua deployments
...
For aqqua, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than an aqqua-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote aqqua server
...
How does an aqqua server come to exist on the target machine?
...
The user or operator already runs aqqua on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in aqqua:
...
The recommended aqqua flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies aqqua availability.
3. Desktop launches or reuses a remote aqqua server.
...
This is the inverse of remote launch: a local aqqua server is already running, and the client publishes it through a tunnel.
...
- A manually hosted aqqua server might be reached through direct `wss`.
...
aqqua already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
aqqua should not copy that part.

aqqua already has the right runtime boundary:

- one aqqua server per environment
- ordinary HTTP/WebSocket between client and environment

So aqqua should borrow Zed's launch discipline, not its transport protocol.
