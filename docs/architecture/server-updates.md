...
aqqua can update a connected server to the exact version of the client that detected version
...
| ----------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `boot-service`    | Linux server running under the aqqua-managed systemd user service                             | Call the update RPC; the service unit is replaced and restarted.      |
...
The systemd user service is a host lifecycle concern, not an aqqua Connect resource. The standalone
`aqqua service install`, `uninstall`, `update`, and `status` commands own it. Install and update both
reconcile the unit through `BootService`; running `npx aqqua@latest service update` therefore pins and
activates the latest CLI release without requiring a connected client.

The `aqqua connect` onboarding flow may offer service installation, but it calls the same reconciliation
operation as `aqqua service install`. Connect logout only disables cloud access and clears its
authorization; it does not uninstall the host service.

## Process Handoff

For `boot-service`, the server atomically rewrites the aqqua-managed user unit to point at the verified
