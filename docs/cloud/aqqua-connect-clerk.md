# aqqua Connect Clerk Setup

aqqua Connect uses one Clerk application for web, desktop, and mobile authentication. The relay accepts
Clerk JWTs only when they are generated from the `aqqua-relay` template with the shared
`aqqua-relay` audience.

## Application Keys

aqqua Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:

```dotenv
AQQUA_CLERK_PUBLISHABLE_KEY=<publishable key>
AQQUA_CLERK_JWT_TEMPLATE=<JWT template name>
AQQUA_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
AQQUA_RELAY_URL=https://relay.example.com
```
...
should set `AQQUA_CLERK_PUBLISHABLE_KEY`, `AQQUA_CLERK_JWT_TEMPLATE`,
`AQQUA_CLERK_CLI_OAUTH_CLIENT_ID`, and `AQQUA_RELAY_URL` before building. EAS preview and
...
absent, the `aqqua connect` CLI command group is omitted. The bundled server still accepts runtime
...
deployment defaults.
`vp run --filter aqqua-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `aqqua connect` commands authorize a headless environment with a separate Clerk OAuth application.
This uses an OAuth public client with PKCE, so the CLI stores no client secret.

In **Clerk Dashboard > OAuth applications**:

1. Create an OAuth application for the aqqua CLI.
...
5. Set `AQQUA_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
aqqua connect login
aqqua connect link
aqqua connect status
aqqua connect unlink
aqqua connect logout
aqqua serve
```

`aqqua connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `aqqua connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running aqqua server. The next `aqqua serve` or `aqqua start` reconciles the relay link and launches the
managed tunnel. `aqqua connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `aqqua connect link` can re-enable exposure without another browser flow. `aqqua connect
...
logout leaves it running; manage it with `aqqua service status`, `install`, `update`, and `uninstall`.

The current OAuth callback listener binds to loopback port `34338`. When running the CLI over SSH,
forward that port before running `aqqua connect login` or `aqqua connect link`:
...
| ------- | -------------------------- |
| Name    | `aqqua-relay`              |
| Claims  | `{ "aud": "aqqua-relay" }` |

Set `AQQUA_CLERK_JWT_TEMPLATE=aqqua-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=aqqua-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `AQQUA_RELAY_URL` still
...
Local desktop development uses `aqqua-dev://app`, while packaged builds use `aqqua://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `aqqua-dev://app`; the production Clerk instance should only need
`aqqua://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `com.aqqua.aqqua`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.aqqua.aqqua` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.aqqua.aqqua`.
...
AQQUA_MACOS_PROVISIONING_PROFILE=/absolute/path/to/aqqua.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
AQQUA_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `AQQUA_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`AQQUA_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `AQQUA_PORT` set. Rebuild the signed app
...
Approved signed-in users manage aqqua Connect under **Connections**. The web and desktop sidebars do
not expose a dedicated account or waitlist control. Signed-out users reach Clerk's waitlist and
sign-in flow contextually from the aqqua Connect controls on the Connections page.

On mobile, signed-out users open **Settings > aqqua Account** to reach `/settings/waitlist` within the
