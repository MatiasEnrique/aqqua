...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `aqqua` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@aqqua/shared/git`, `@aqqua/shared/DrainableWorker`) — no barrel index.
