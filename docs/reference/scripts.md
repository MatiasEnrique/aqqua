...
- Dev commands run from a linked **git worktree** default to that worktree's gitignored `.aqqua`, even when `AQQUA_HOME` is set, storing state in `<worktree>/.aqqua/userdata`. Pass `--home-dir <path>` to choose another isolated directory explicitly. Submodules are not worktrees and keep the normal precedence.
- From the **main checkout**, dev commands implicitly use `~/.aqqua/dev`, keeping development state separate from `~/.aqqua/userdata`. An explicit `--home-dir <path>` stores state under `<path>/userdata`; the base directory remains available for caches, worktrees, and other shared data.
- Web dev commands do not auto-open a browser. Open the one-time pairing URL printed by the server so the first browser navigation is authenticated. Set `AQQUA_NO_BROWSER=0` only when interactive auto-open is intentional.
- Pass dev-runner flags directly after the root task name, for example:
  `vp run dev --home-dir /tmp/aqqua-dev`
...
- `node apps/server/scripts/aqqua-sqlite-state.ts <query|exec> --base-dir <path> ...` — Inspects or seeds an isolated aqqua SQLite database; writes create a private backup first.
...
- Desktop production windows load the bundled UI from `aqqua://app/index.html` (not a `127.0.0.1` document URL).

- Desktop packaging includes `apps/server/dist` (the `aqqua` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
- Signed macOS builds also require `AQQUA_APPLE_TEAM_ID` and
  `AQQUA_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `AQQUA_CLERK_PUBLISHABLE_KEY` unless `AQQUA_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
Set `AQQUA_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `13773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `AQQUA_DEV_INSTANCE`)
- Example: `AQQUA_DEV_INSTANCE=branch-a vp run dev:desktop`

If you want full control instead of hashing, set `AQQUA_PORT_OFFSET` to a numeric offset.
