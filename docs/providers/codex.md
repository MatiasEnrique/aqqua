...
This guide is for people who want to use more than one Codex account in aqqua.
...
- both accounts can see the same aqqua/Codex sessions
...
In aqqua Settings, name it something obvious:
...
In aqqua Settings, add another Codex provider:
...
aqqua shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, aqqua treats it
