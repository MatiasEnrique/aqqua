...
Define the long-term server auth architecture for aqqua before first-class remote environments ship.
...
Every privileged surface of the aqqua server must go through the same auth policy engine:
...
Participants:
  ...
  AqquaServer      = local backend child process
...
DesktopMain -> AqquaServer : spawn server (--bootstrap-fd ...)
DesktopMain -> AqquaServer : send desktop bootstrap envelope
note over AqquaServer : policy = DesktopManagedLocalPolicy
note over AqquaServer : allowed pairing = desktop-bootstrap only

Frontend -> DesktopMain : request local bootstrap grant
DesktopMain --> Frontend : short-lived desktop bootstrap grant

Frontend -> AqquaServer : POST /api/auth/bootstrap
AqquaServer -> AqquaServer : validate desktop bootstrap grant
AqquaServer -> AqquaServer : create browser session
AqquaServer --> Frontend : Set-Cookie: session=...

Frontend -> AqquaServer : GET /ws + authenticated cookie
AqquaServer -> AqquaServer : validate cookie session
AqquaServer --> Frontend : websocket accepted
```
...
```text
Participants:
  UserShell     = npx aqqua launcher
  AqquaServer      = standalone local server
  Browser       = browser tab

UserShell -> AqquaServer : start server
AqquaServer -> AqquaServer : getOrCreate("server-signing-key")
note over AqquaServer : policy = LoopbackBrowserPolicy

UserShell -> AqquaServer : issue one-time pairing token
AqquaServer --> UserShell : pairing URL or pairing token

UserShell --> Browser : open /pair?token=...

Browser -> AqquaServer : GET /pair?token=...
AqquaServer -> AqquaServer : validate one-time token
AqquaServer -> AqquaServer : create browser session
AqquaServer --> Browser : Set-Cookie: session=...
AqquaServer --> Browser : redirect to app

Browser -> AqquaServer : GET /ws + authenticated cookie
AqquaServer --> Browser : websocket accepted
```
...
Participants:
  ...
  AqquaServer      = aqqua server
  PhoneBrowser  = mobile browser

DesktopUser -> DesktopMain : enable remote access via tunnel
DesktopMain -> AqquaServer : switch policy to RemoteReachablePolicy
DesktopMain -> Tunnel : publish local aqqua endpoint
Tunnel --> DesktopMain : public https/wss URL

DesktopMain -> AqquaServer : issue one-time pairing token
AqquaServer --> DesktopMain : pairing token
...
Tunnel -> AqquaServer : forward request
AqquaServer -> AqquaServer : validate one-time token
AqquaServer -> AqquaServer : create mobile browser session
AqquaServer --> PhoneBrowser : Set-Cookie: session=...
AqquaServer --> PhoneBrowser : redirect to app

PhoneBrowser -> Tunnel : GET /ws + authenticated cookie
Tunnel -> AqquaServer : forward websocket upgrade
AqquaServer --> PhoneBrowser : websocket accepted
```
...
DesktopUser -> AqquaServer : enable private-network access
AqquaServer -> AqquaServer : switch policy to RemoteReachablePolicy
DesktopUser -> AqquaServer : issue one-time pairing token
AqquaServer --> DesktopUser : pairing URL / QR

DesktopUser -> PhoneBrowser : open private-network URL
PhoneBrowser -> PrivateNet : GET http(s)://private-host/pair?token=...
PrivateNet -> AqquaServer : route request
AqquaServer -> AqquaServer : validate one-time token
AqquaServer -> AqquaServer : create mobile browser session
AqquaServer --> PhoneBrowser : Set-Cookie: session=...
AqquaServer --> PhoneBrowser : redirect to app
...
PrivateNet -> AqquaServer : websocket upgrade
AqquaServer --> PhoneBrowser : websocket accepted
```
...
Participants:
  ...
  RemoteAqqua      = remote aqqua server
  Frontend      = desktop renderer

DesktopUser -> DesktopMain : add SSH host
DesktopMain -> SSH : connect to remote host
SSH -> RemoteHost : probe environment / verify aqqua availability
DesktopMain -> SSH : run remote launch command
SSH -> RemoteHost : aqqua remote launch --json
RemoteHost -> RemoteAqqua : start or reuse server
RemoteAqqua --> RemoteHost : port + environment metadata
...
note over RemoteAqqua : policy = RemoteReachablePolicy
note over DesktopMain,RemoteAqqua : desktop may use a trusted bootstrap flow here

Frontend -> DesktopMain : request bootstrap for selected environment
DesktopMain --> Frontend : short-lived bootstrap grant

Frontend -> RemoteAqqua : POST /api/auth/bootstrap via forwarded port
RemoteAqqua -> RemoteAqqua : validate bootstrap grant
RemoteAqqua -> RemoteAqqua : create browser session
RemoteAqqua --> Frontend : Set-Cookie: session=...

Frontend -> RemoteAqqua : GET /ws + authenticated cookie
RemoteAqqua --> Frontend : websocket accepted
```
