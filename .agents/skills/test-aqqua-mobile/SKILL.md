---
name: test-aqqua-mobile
description: Launch and test aqqua Mobile on an iOS Simulator or Android Emulator against disposable local aqqua environments, including Metro and dev-client reuse, native rebuild decisions, per-client pairing, seeded projects, semantic UI control, screenshots, and iOS serve-sim streaming. Use after mobile UI or native changes, when reproducing phone or tablet behavior, pairing an emulator to isolated state, or verifying mobile behavior on macOS, Linux, or Windows.
---

# Test aqqua Mobile

Run one focused, end-to-end mobile verification pass against disposable aqqua state. Use the sibling [`test-aqqua-app`](../test-aqqua-app/SKILL.md) skill as the detailed reference for pairing-token semantics and SQLite fixtures.
...
- App: `aqqua Dev`
- Bundle/package identifier: `com.aqqua.aqqua.dev`
- URL scheme: `aqqua-dev`
...
## Start one disposable aqqua environment

Run backend commands from the repository root. Use the ignored, worktree-local `.aqqua` directory or create a fresh directory with the host OS's temporary-directory mechanism. An explicit base directory stores state in `<base-dir>/userdata`; never point testing at shared `~/.aqqua` state.
...
Use direct SQLite mutation only for disposable projection fixtures. Follow `test-aqqua-app` and stop the backend before writing.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `aqqua-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/AqquaDev.xcworkspace`
- Scheme: `AqquaDev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `com.aqqua.aqqua.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.aqqua.aqqua.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.aqqua.aqqua.dev
...
In PowerShell, set `$env:AQQUA_PORT = "<server-port>"` first and run the `node ... auth pairing create` command without the leading assignment.
...
```bash
xcrun simctl openurl <simulator-udid> 'aqqua-dev://connections/new'
adb -s <emulator-serial> shell am start -W \
  -a android.intent.action.VIEW \
  -d 'aqqua-dev://connections/new' \
  com.aqqua.aqqua.dev
```

Run only the command for the selected platform.

In aqqua Dev, open Add Environment and enter the complete `<mobile-origin>` and newly printed `Token`. Verify the expected seeded projects appear before exercising the affected flow.
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in aqqua when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from aqqua Dev.
