...
Load this reference only when inspecting or seeding local aqqua state directly.
...
```bash
node apps/server/scripts/aqqua-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/aqqua-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/aqqua-seed.sql
```
