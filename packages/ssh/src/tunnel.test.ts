...
describe("ssh tunnel scripts", () => {
  it("builds the remote aqqua runner with npx and npm fallbacks", () => {
    const script = buildRemoteAqquaRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "AQQUA_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec aqqua "$@"');
    assert.include(script, "exec npx --yes 'aqqua@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'aqqua@latest' -- \"$@\"");
    assert.include(script, "could not install 'aqqua@latest'");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `AQQUA_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for AQQUA_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
  ...
  it("does not hard-code a remote node engine range", () => {
    ...
    assert.include(script, "AQQUA_NODE_ENGINE_RANGE=''");
  ...
  it("builds the remote aqqua runner with a node script override", () => {
    ...
    assert.include(
      script,
      "AQQUA_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$AQQUA_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote aqqua runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `AQQUA_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
