...
import { applyGitStatusStreamEvent } from "@aqqua/shared/git";
...
export function createVcsEnvironmentAtoms<R, E>(
  ...
  return {
    listHistory: createEnvironmentRpcQueryAtomFamily(runtime, {
      label: "environment-data:vcs:list-history",
      tag: WS_METHODS.vcsListHistory,
      staleTimeMs: 0,
    }),
    ...
    inspectWorktreeRemoval: createEnvironmentRpcCommand(runtime, {
      label: "environment-data:vcs:inspect-worktree-removal",
      tag: WS_METHODS.vcsInspectWorktreeRemoval,
      scheduler: vcsCommandScheduler,
      concurrency: vcsCommandConcurrency,
    }),
