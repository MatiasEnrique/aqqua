...
}).annotate({ description: "Links an authenticated cloud user to an aqqua environment." });
...
>()("@aqqua/contracts/relay/RelayClientPrincipal") {}
...
>()("@aqqua/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in aqqua Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["aqqua-mobile", "aqqua-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "aqqua-mobile" as const;
export const RelayWebClientId = "aqqua-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "aqqua Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking aqqua environments, connecting authorized clients, and publishing agent activity.",
  );
