...
import { resolveGitWorktreePath, resolveWorktreeAqquaHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "aqqua-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeAqquaHome", () => {
  it.effect("answers with .aqqua before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeAqquaHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".aqqua"));
