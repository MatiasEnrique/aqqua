...
> [!WARNING]
> aqqua Connect is currently in private beta. Join the waitlist in the app under Settings > aqqua Connect.

The relay is the hosted control plane for aqqua Connect. It helps clients discover and connect to
...
The relay is intentionally not in the hot path for normal aqqua traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [aqqua Connect architecture overview](../../docs/cloud/aqqua-connect-auth-flow.html) for the larger system
design.
...
- Linking aqqua environments to a cloud account.
...
```sh
vp run --filter aqqua-relay deploy
```
...
```sh
vp run --filter aqqua-relay deploy -- --stage prod
vp run --filter aqqua-relay deploy -- --env-file .env.local
```
...
- [aqqua Connect Clerk Setup](../../docs/cloud/aqqua-connect-clerk.md) for Clerk keys, JWT templates, and waitlist
  setup.
- [Relay Observability](../../docs/relay-observability.md) for deployment tracing and diagnostics.
- [aqqua Connect Architecture Overview](../../docs/cloud/aqqua-connect-auth-flow.html) for the full link,
