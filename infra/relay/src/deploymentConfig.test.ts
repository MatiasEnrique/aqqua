...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("aqqua-relay-traces", "prod")).toBe("aqqua-relay-traces-prod");
    expect(relayResourceNameForStage("aqqua-relay-traces", "dev_julius")).toBe(
      "aqqua-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "aqquarelay.com", hash)).toBe(
      "prod-abcdef0123456789.aqquarelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "aqquarelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
