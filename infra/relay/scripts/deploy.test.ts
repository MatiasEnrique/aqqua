...
describe("reconcileRootEnvRelayUrl", () => {
  ...
  it("preserves unrelated root env entries while replacing a previous relay URL", () => {
    expect(
      reconcileRootEnvRelayUrl(
        "AQQUA_CLERK_PUBLISHABLE_KEY=pk_test_example\nAQQUA_RELAY_URL=https://old.example.test\n",
...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "aqqua-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "aqqua-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "AQQUA_MOBILE_OTLP_TRACES_DATASET=aqqua-mobile-traces-dev",
        "AQQUA_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "AQQUA_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "AQQUA_RELAY_CLIENT_OTLP_TRACES_DATASET=aqqua-relay-client-traces-dev",
        "AQQUA_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
    ...
    expect(
      reconcileRootEnvPublicConfig(
        [
          "AQQUA_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "AQQUA_RELAY_URL=https://old.example.test",
          "AQQUA_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "AQQUA_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "AQQUA_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "AQQUA_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "AQQUA_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "AQQUA_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "AQQUA_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "AQQUA_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "AQQUA_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
